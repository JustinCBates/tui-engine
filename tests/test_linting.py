...
def _ensure_tool_present(executable_name: str):
    """Skip the test if the given executable is not present on PATH or in venv.
    ...
    venv_path = Path(sys.executable).parent / executable_name
    if venv_path.exists():
        return
...
def test_ruff_check():
    ...
    proc_src = _run_cmd(
        [sys.executable, "-m", "ruff", "check", str(repo_root / "src")], cwd
    )
    proc_tests = _run_cmd(
        [sys.executable, "-m", "ruff", "check", str(repo_root / "tests")], cwd
    )
    ...
    assert (
        proc_src.returncode == 0 and proc_tests.returncode == 0
    ), "ruff reported issues; see output above"
...
def test_black_check():
    ...
    proc_src = _run_cmd(
        [sys.executable, "-m", "black", "--check", str(repo_root / "src")], cwd
    )
    proc_tests = _run_cmd(
        [sys.executable, "-m", "black", "--check", str(repo_root / "tests")], cwd
    )
    if proc_src.returncode != 0:
        print(proc_src.stdout)
    if proc_tests.returncode != 0:
    ...
    assert (
        proc_src.returncode == 0 and proc_tests.returncode == 0
    ), "black --check reported formatting issues; see output above"
