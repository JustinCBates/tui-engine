...
class TestQuestionaryBridgeCompatibility:
    ...
    def test_questionary_api_equivalence(self, monkeypatch):
        ...
        components = [
            ...
            Component(name="s1", component_type="select", message="Select prompt", choices=["a", "b"]),
            Component(name="c1", component_type="confirm", message="Confirm prompt"),
            Component(name="cb1", component_type="checkbox", message="Checkbox prompt", choices=["x", "y"])
        ]
...
class TestQuestionaryBridgeEndToEnd:
    ...
    def test_full_form_workflow(self, monkeypatch):
        ...
        form_components = [
            ...
            Component(name="subscribe", component_type="confirm", message="Subscribe to newsletter?"),
            Component(name="interests", component_type="checkbox", message="Select interests:",
    ...
    def test_conditional_workflow(self, monkeypatch):
        ...
        mock_q.checkbox = lambda *a, **k: types.SimpleNamespace(ask=lambda: ["default_choice"])
        mock_q.password = lambda *a, **k: types.SimpleNamespace(ask=lambda: "default_password")
        mock_q.autocomplete = lambda *a, **k: types.SimpleNamespace(ask=lambda: "default_auto")
        ...
        if signup_result:
            plan = Component(name="plan", component_type="select", 
                           message="Choose plan:", choices=["basic", "premium"])
            plan_result = bridge.ask_component(plan)
            
            if plan_result == "premium":
                billing = Component(name="billing", component_type="select",
                                  message="Billing cycle:", choices=["monthly", "yearly"])
                billing_result = bridge.ask_component(billing)
    ...
    def test_error_recovery_workflow(self, monkeypatch):
        ...
        mock_q.select = lambda *a, **k: types.SimpleNamespace(ask=lambda: "default_option")
        mock_q.confirm = lambda *a, **k: types.SimpleNamespace(ask=lambda: True)
        mock_q.checkbox = lambda *a, **k: types.SimpleNamespace(ask=lambda: ["default_choice"])
        mock_q.password = lambda *a, **k: types.SimpleNamespace(ask=lambda: "default_password")
        mock_q.autocomplete = lambda *a, **k: types.SimpleNamespace(ask=lambda: "default_auto")
        ...
        component = Component(name="retry_test", component_type="text", message="Enter value:")
...
class TestQuestionaryBridgeModuleIntegration:
    ...
    def test_bridge_with_minimal_component(self, monkeypatch):
        ...
        minimal_component = Component(name="minimal_test", component_type="text", message="Enter value:")
    ...
    def test_bridge_load_and_exercise_standalone(self):
        ...
        bridge_path = os.path.join(here, "src", "questionary_extended", "integration", "questionary_bridge.py")
        ...
        if os.path.exists(bridge_path):
            with open(bridge_path, "r", encoding="utf8") as fh:
                content = fh.read()
