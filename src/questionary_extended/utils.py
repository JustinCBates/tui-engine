...
def sanitize_input(text: str, allowed_chars: Optional[str] = None) -> str:
    ...
    sanitized = re.sub(r'[\x01-\x08\x0B\x0C\x0E-\x1F\x7F]', '', sanitized)  # Control chars
    
    return sanitized


def fuzzy_match(query: str, choices: List[str], threshold: float = 0.6) -> List[Tuple[str, float]]:
