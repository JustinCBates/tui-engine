...
def enhanced_text(
    message: str, default: str = "", multiline: bool = False, validator=None, **kwargs
) -> Question:
    """
    ...
    return questionary.text(message, default=default, multiline=multiline, **kwargs)
...
def number(
    ...
    **kwargs,
    ...
    return questionary.text(
        message, default=default_str, validate=validator, **clean_kwargs
    )
...
class progress_tracker:
    ...
    def step(self, description: str):
        ...
        print(
            f"   [{bar}] {self.current_step}/{self.total_steps} ({progress:.1f}%) - {description}"
        )
...
def select_enhanced(message: str, choices: List[Any], **kwargs) -> Question:
    """Enhanced selection prompt."""
    return questionary.select(message, choices=choices, **kwargs)


def checkbox_enhanced(message: str, choices: List[Any], **kwargs) -> Question:
