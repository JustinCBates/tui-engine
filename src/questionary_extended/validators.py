...
class NumberValidator(Validator):
    ...
    def validate(self, document):
        ...
        try:
            ...
            raise ValidationError(
                message=f"Please enter a valid {number_type}",
...
class DateValidator(Validator):
    ...
    def validate(self, document):
        ...
        if self.min_date and parsed_date < self.min_date:
            raise ValidationError(
                message=f"Date must be after {self.min_date.strftime(self.format_str)}",
                cursor_position=len(document.text),
            )

        if self.max_date and parsed_date > self.max_date:
            raise ValidationError(
                message=f"Date must be before {self.max_date.strftime(self.format_str)}",
                cursor_position=len(document.text),
            )
...
class URLValidator(Validator):
    ...
    def validate(self, document):
        ...
        if not text:
            raise ValidationError(
                message="Please enter a URL", cursor_position=len(document.text)
            )

        if self.require_https and not text.startswith("https://"):
            raise ValidationError(
                message="URL must use HTTPS", cursor_position=len(document.text)
            )
...
class RangeValidator(Validator):
    ...
    def validate(self, document):
        ...
        try:
            ...
            raise ValidationError(
                message="Invalid value format", cursor_position=len(document.text)
            )
        ...
        if self.inclusive:
            ...
            if value <= self.min_val or value >= self.max_val:
                raise ValidationError(
                    message=f"Value must be between {self.min_val} and {self.max_val} (exclusive)",
                    cursor_position=len(document.text),
                )
...
class ChoiceValidator(Validator):
    ...
    def validate(self, document):
        ...
        if not text:
            raise ValidationError(
                message="Please select a value", cursor_position=len(document.text)
            )
        ...
        if check_text not in self.choices:
            ...
            raise ValidationError(
                message=f"Please choose from: {choices_str}",
                cursor_position=len(document.text),
            )
