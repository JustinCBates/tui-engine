...
def demo_autocomplete_features():
    ...
    lang = questionary.autocomplete("Language (type to filter):", choices=languages).ask()
    print(f"   Result: {lang}")
...
def demo_url_validator_features():
    ...
    url2 = questionary.text("Enter HTTPS URL:", validate=URLValidator(require_https=True)).ask()
...
def demo_date_validator_features():
    ...
    date1 = questionary.text(
        "Date (YYYY-MM-DD):",
        validate=DateValidator(format_str="%Y-%m-%d")
    ).ask()
...
def demo_format_number_features():
    ...
    print(f"   Percentage: {qe.format_number(0.1567, percentage=True, decimal_places=1)}")
...
def demo_progress_tracker_features():
    ...
    with qe.progress_tracker("Basic Task", total_steps=3) as progress:
    ...
    try:
        with qe.progress_tracker("Task with Error", total_steps=2) as progress:
...
def demo_number():
    """Demo number input."""
    age = qe.number("Enter your age:", min_value=0, max_value=150, allow_float=False).ask()
    if age:
...
def demo_autocomplete():
    ...
    language = questionary.autocomplete(
        "Choose a programming language (type to filter):",
        choices=languages
    ).ask()
...
def demo_progress_tracker():
    ...
    with qe.progress_tracker("Demo Process", total_steps=4) as progress:
        progress.step("Initializing...")
        time.sleep(0.8)
...
def run_full_demo():
    ...
    with qe.progress_tracker("Full Demo", total_steps=6) as progress:
