...
def build_user_profile_form():
    ...
    form_data = questionary.form(
        full_name=questionary.text(
            "Full Name:", validate=lambda text: len(text) > 0 or "Name is required"
        ),
        email=questionary.text("Email Address:", validate=EmailValidator()),
        age=qe.number("Age:", min_value=18, max_value=120, allow_float=False),
        ...
        work_style=questionary.select(
            "Preferred work style:", choices=["Remote", "Hybrid", "On-site", "Flexible"]
        ),
        open_to_relocation=questionary.confirm("Open to relocation?", default=False),
    ).ask()
...
def build_project_setup_wizard():
    ...
    framework = questionary.select(
        "Choose framework:", choices=framework_choices.get(project_type, [])
    ).ask()
...
def main():
    ...
    with qe.ProgressTracker("Project Setup", total_steps=4) as progress:
    ...
    overall_rating = qe.rating(
        "How would you rate this experience?", max_rating=5, icon="⭐"
    ).ask()
